...
function Trans(conn, collection, defaultModel, options) {
    let _trans, models = {};
    var _options = Object.assign({}, options);
    if (_options.done_mode != "DEL") {
        _options.done_mode = "SUCCESS";
    }
    ...
    this.__defineGetter__("done", function () {
        return async function (id, queue) {
            ...
            let origin;
            if (_options.done_mode == "SUCCESS") {
                origin = await _trans.findByIdAndUpdate(m, id, { state: "success" });
            }
            else if (_options.done_mode == "DEL") {
                origin = await _trans.findByIdAndRemove(m, id);
            }
 ...
 * @param {enum} options.done_mode - 消息完成的处理方式，可选值：DEL/SUCCESS，删除消息和标识为已完成。
 */
function Queue(connectionstring, name, options) {
    ...
    async function Begin() {
        let trans = new Trans(conn, "_trans", model, _options);
